                        ...
                        <div className="space-y-3">
                            <div className="space-y-1">
                                ...
                                <div className="flex gap-1">
                                    <input required type="date" className="w-full border rounded-lg p-2.5 text-sm font-bold" value={startDate} onChange={e => setStartDate(e.target.value)} />
                                    <input type="time" className="w-24 border rounded-lg p-2.5 text-sm font-bold" value={startTime} onChange={e => setStartTime(e.target.value)} />
                                </div>
                            </div>
                            <div className="space-y-1">
                                <label className="text-[10px] font-black text-slate-400 uppercase">Selesai Sewa</label>
                                <div className="flex gap-1">
                                    <input required type="date" className="w-full border rounded-lg p-2.5 text-sm font-bold" value={endDate} onChange={e => setEndDate(e.target.value)} />
                                    <input type="time" className="w-24 border rounded-lg p-2.5 text-sm font-bold" value={endTime} onChange={e => setEndTime(e.target.value)} />
                                </div>
                            </div>
                        </div>
                    ...
                    return (
                        <div key={b.id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all flex flex-col justify-between gap-6 relative overflow-hidden">
                            ...
                            <div className="flex flex-wrap gap-2 w-full justify-start border-t pt-4">
                                ...
                                <button onClick={() => window.open(generateWhatsAppLink(b, car!), '_blank')} className="p-2.5 bg-green-50 text-green-600 rounded-xl hover:bg-green-100 transition-colors border border-green-100"><MessageCircle size={20}/></button>
                                <button onClick={() => generateInvoicePDF(b, car!)} className="p-2.5 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-100 transition-colors border border-blue-100"><Zap size={20}/></button>
