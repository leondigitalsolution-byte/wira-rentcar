...
// Added Car as CarIcon to fix the error: Cannot find name 'CarIcon'
import { Plus, Trash2, Edit2, User as UserIcon, Car as CarIcon, Upload, X, Download, FileSpreadsheet, MapPin, Camera, DollarSign, List, Info, Search, Filter } from 'lucide-react';
...
const FleetPage: React.FC<Props> = ({ currentUser }) => {
  ...
  const filteredCarsList = useMemo(() => {
    ...
    if (filterCategory !== 'All') {
        result = result.filter(c => c.type === filterCategory);
    }
  ...
  const handleExport = () => exportToCSV(filteredCarsList, 'Data_Armada_WiraRentCar');
    ...
    <div className="space-y-6">
      ...
      <div className="sticky top-0 z-20 -mx-4 md:-mx-8 px-4 md:px-8 py-4 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-700 shadow-sm">
            ...
            <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                    <div className="hidden md:flex gap-2">
                        ...
                        <button onClick={handleImportClick} title="Import CSV" className="bg-white dark:bg-slate-800 border dark:border-slate-700 text-slate-700 dark:text-slate-200 p-2 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                            <Upload size={20} />
                        </button>
                        <button onClick={handleExport} title="Export CSV" className="bg-white dark:bg-slate-800 border dark:border-slate-700 text-slate-700 dark:text-slate-200 p-2 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                            <Download size={20} />
                        </button>
                    </div>
                    {!isPartnerView && (
          ...
          <div className="flex flex-wrap items-center gap-3 mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                ...
                {/* Category Filter */}
                <select 
                    className="bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-lg px-3 py-1.5 text-xs font-bold text-slate-600 dark:text-slate-300 outline-none"
                ...
                {(filterCategory !== 'All' || filterBrand !== 'All' || filterOwner !== 'All' || searchTerm !== '') && (
                    <button 
                        onClick={() => { setFilterCategory('All'); setFilterBrand('All'); setFilterOwner('All'); setSearchTerm(''); }}
            ...
            <div key={car.id} className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden hover:shadow-xl transition-all group animate-in fade-in zoom-in-95 duration-200">
                <div className="h-52 bg-slate-200 dark:bg-slate-900 relative overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3 flex flex-col gap-2">
                        <span className="bg-white/90 dark:bg-slate-800/90 backdrop-blur px-3 py-1 rounded-full text-[10px] font-black uppercase text-indigo-700 dark:text-indigo-400 shadow-sm">
                            {car.type}
                        </span>
                        {car.brand && (
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
              <div className="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-2xl p-6 shadow-2xl max-h-[90vh] overflow-hidden flex flex-col border border-slate-200 dark:border-slate-700">
                  <div className="flex justify-between items-center mb-4">
                      ...
                      <button onClick={() => setIsModalOpen(false)} className="text-slate-400 hover:text-slate-600 bg-slate-100 dark:bg-slate-700 p-1 rounded-full">
                          <X size={24} />
                      </button>
                  </div>
                  ...
                  <div className="flex gap-2 p-1 bg-slate-100 dark:bg-slate-900 rounded-xl mb-6">
                      <button onClick={() => setModalTab('umum')} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all flex items-center justify-center gap-2 ${modalTab === 'umum' ? 'bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-500'}`}>
                          <Info size={16}/> Umum
                      </button>
                      <button onClick={() => setModalTab('harga')} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all flex items-center justify-center gap-2 ${modalTab === 'harga' ? 'bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-500'}`}>
                  ...
                  <form onSubmit={handleSave} className="space-y-6 overflow-y-auto flex-1 pr-2 custom-scrollbar">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in slide-in-from-left-2">
                              ...
                              <div className="space-y-4">
                                  <div>
                                      ...
                                      <input required type="text" placeholder="Veloz" className="w-full border dark:border-slate-700 dark:bg-slate-900 dark:text-white rounded-xl p-2.5 mt-1 font-bold" value={name} onChange={e => setName(e.target.value)} />
                                  </div>
                                  <div>
                                      <label className="block text-xs font-black uppercase text-slate-400 tracking-widest">Merek Mobil</label>
                                      <input required type="text" placeholder="Toyota" className="w-full border dark:border-slate-700 dark:bg-slate-900 dark:text-white rounded-xl p-2.5 mt-1 font-bold uppercase" value={brand} onChange={e => setBrand(e.target.value)} />
                                  </div>
                                  <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-xs font-black uppercase text-slate-400 tracking-widest">Plat Nomor</label>
                                        <input required type="text" placeholder="B 1234 XX" className="w-full border dark:border-slate-700 dark:bg-slate-900 dark:text-white rounded-xl p-2.5 mt-1 font-bold font-mono uppercase" value={plate} onChange={e => setPlate(e.target.value)} />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-black uppercase text-slate-400 tracking-widest">Kategori</label>
                                        <select className="w-full border dark:border-slate-700 dark:bg-slate-900 dark:text-white rounded-xl p-2.5 mt-1 font-bold" value={type} onChange={e => setType(e.target.value)}>
                                  ...
                                  <div>
                                      ...
                                      <select disabled={isPartnerView} className="w-full border dark:border-slate-700 dark:bg-slate-900 dark:text-white rounded-xl p-2.5 mt-1 font-bold" value={partnerId} onChange={e => setPartnerId(e.target.value)}>
                          ...
                          <div className="space-y-6 animate-in fade-in slide-in-from-right-2">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                  <div className="p-4 bg-purple-50 dark:bg-purple-900/10 border border-purple-100 dark:border-purple-900/30 rounded-2xl">
                                      <label className="block text-[10px] font-black uppercase text-purple-600 dark:text-purple-400 mb-1">Setoran ke Investor / hari</label>
                                      <div className="relative">
                                          <span className="absolute left-3 top-2 text-sm font-bold text-purple-400">Rp</span>
                                          <input type="number" className="w-full border-none bg-white dark:bg-slate-900 dark:text-white rounded-xl p-2 pl-10 font-black text-purple-700 dark:text-purple-300" value={investorSetoran} onChange={e => setInvestorSetoran(Number(e.target.value))} placeholder="0" />
                                      </div>
                                  ...
                                  <div className="p-4 bg-orange-50 dark:bg-orange-900/10 border border-orange-100 dark:border-orange-900/30 rounded-2xl">
                                      <label className="block text-[10px] font-black uppercase text-orange-600 dark:text-orange-400 mb-1">Gaji Driver / hari</label>
                                      <div className="relative">
                                          <span className="absolute left-3 top-2 text-sm font-bold text-orange-400">Rp</span>
                                          <input type="number" className="w-full border-none bg-white dark:bg-slate-900 dark:text-white rounded-xl p-2 pl-10 font-black text-orange-700 dark:text-orange-300" value={driverSalary} onChange={e => setDriverSalary(Number(e.target.value))} placeholder="150000" />
                                      </div>
                              ...
                              <div className="space-y-4">
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-700">
                                    {settings.rentalPackages.map(pkg => (
                                        <div key={pkg}>
                                            <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1">{pkg}</label>
                                            <div className="relative">
                                                <span className="absolute left-3 top-2 text-sm font-bold text-slate-400">Rp</span>
                                                <input type="number" className="w-full border dark:border-slate-700 dark:bg-slate-800 dark:text-white rounded-xl p-2 pl-10 font-bold focus:ring-2 ring-indigo-100 dark:ring-indigo-900 outline-none" value={prices[pkg] || ''} onChange={e => handlePriceChange(pkg, Number(e.target.value))} placeholder="0" />
                                            </div>
                                        </div>
                                    ))}
                                  </div>
                      ...
                      <div className="flex gap-3 pt-6 border-t dark:border-slate-700 mt-auto">
                          <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 py-3 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold rounded-xl hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors">Batal</button>
